...
class Plot3DScene:
    def __init__(self, face_model: np.ndarray, screen_width_mm: int, screen_height_mm: int, screen_height_mm_offset: int):
        ...
        # Use Figure.show() in non-interactive mode
        # In newer matplotlib, we need to be more explicit with backends
        try:
            self.fig.show()
            self.fig.canvas.draw()
        except Exception as e:
            print(f"Warning: could not show figure: {e}")
    ...
    def __plot_screen(self) -> None:
        ...
        screen_y = [self.screen_height_mm_offset,
                    self.screen_height_mm + self.screen_height_mm_offset]
        self.ax.plot(
    ...
    def plot_face_landmarks(self, face_landmarks: np.ndarray) -> None:
        if self.face_scatter is None:
            self.face_scatter = self.ax.plot(face_landmarks[0, :], face_landmarks[1, :], face_landmarks[2, :],
                                             linestyle="", marker="o", color='#7f7f7f', markersize=1, label='face landmarks')[0]
        else:
            self.face_scatter.set_data_3d(
                face_landmarks[0, :], face_landmarks[1, :], face_landmarks[2, :])

    def plot_center_point(self, center_point: np.ndarray, gaze_vector_3d_normalized) -> None:
        point = center_point.reshape(
            3) + 1000 * gaze_vector_3d_normalized.reshape(3)

        if self.center_point is None:
            self.center_point = self.ax.plot([center_point[0], point[0]], [center_point[1], point[1]], [
                                             center_point[2], point[2]], color='#2ca02c', label='gaze vector')[0]
        else:
            self.center_point.set_data_3d([center_point[0], point[0]], [
                                          center_point[1], point[1]], [center_point[2], point[2]])

    def plot_point_on_screen(self, point_on_screen):
        if self.point_on_screen is None:
            self.point_on_screen = self.ax.plot(point_on_screen[0], point_on_screen[1], point_on_screen[2],
                                                linestyle="", marker="X", color='#9467bd', markersize=5, label='target on screen')[0]
        else:
            self.point_on_screen.set_data_3d(
                point_on_screen[0], point_on_screen[1], point_on_screen[2])
    ...
    def update_canvas(self) -> None:
        ...
        # Handle potential errors if running in a headless environment
        try:
            self.fig.canvas.draw()
            self.fig.canvas.flush_events()
        except Exception as e:
            # Silent fail if running headless
            pass
