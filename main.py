...
from qt_visualizer import GazeVisualizerGUI
...
# Global flag for GUI support
HAS_GUI_SUPPORT = has_gui_support()

# Safe versions of OpenCV GUI functions


def safe_namedWindow(window_name, flag=cv2.WINDOW_AUTOSIZE):
    """Create a named window only if GUI support is available."""
    if HAS_GUI_SUPPORT:
        return cv2.namedWindow(window_name, flag)
    else:
        print(f"Skipping window creation for '{window_name}' (no GUI support)")
        return None


def safe_imshow(window_name, image):
    """Show an image only if GUI support is available."""
    if HAS_GUI_SUPPORT:
...
def safe_setWindowProperty(window_name, prop_id, prop_value):
    """Set window property only if GUI support is available."""
    if HAS_GUI_SUPPORT:
        return cv2.setWindowProperty(window_name, prop_id, prop_value)
    else:
        return None
...
def safe_waitKey(delay):
    """Wait for a key press only if GUI support is available."""
    if HAS_GUI_SUPPORT:
        return cv2.waitKey(delay) & 0xFF
    else:
        # Simulate a small delay in headless mode
        time.sleep(delay/1000)
        return 0xFF  # Return a value that won't match any key code


def fix_checkpoint_compatibility(checkpoint_path):
    """
    Fix PyTorch Lightning checkpoint compatibility issues by adding missing keys.

    Returns the path to the fixed checkpoint file.
    """
    print(f"Loading checkpoint with compatibility fixes: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu'))

    # Add missing keys for PyTorch Lightning compatibility
    if "pytorch-lightning_version" not in checkpoint:
        print("Adding missing 'pytorch-lightning_version' to checkpoint")
        checkpoint["pytorch-lightning_version"] = "1.5.0"

    if "global_step" not in checkpoint:
        print("Adding missing 'global_step' to checkpoint")
        checkpoint["global_step"] = 0

    if "epoch" not in checkpoint:
        print("Adding missing 'epoch' to checkpoint")
        checkpoint["epoch"] = 0

    if "hyper_parameters" not in checkpoint:
        print("Adding missing 'hyper_parameters' to checkpoint")
        checkpoint["hyper_parameters"] = {}

    # Save the fixed checkpoint
    temp_path = checkpoint_path + '.fixed'
    torch.save(checkpoint, temp_path)
    print(f"Saved fixed checkpoint to: {temp_path}")
    return temp_path
...
def main(calibration_matrix_path=None, monitor_mm=None, monitor_pixels=None, model=None, visualize_preprocessing=False, visualize_laser_pointer=True, visualize_3d=False):
    ...
    # Initialize camera matrix as identity if no calibration file
    if calibration_matrix_path:
        camera_matrix, dist_coefficients = get_camera_matrix(
            calibration_matrix_path)
    else:
        # Use default camera matrix if no calibration file
        camera_matrix = np.array(
            [[1000, 0, 640], [0, 1000, 360], [0, 0, 1]], dtype=np.float32)
        dist_coefficients = np.zeros((1, 5), dtype=np.float32)
        print("Using default camera matrix without calibration")
    ...
    if monitor_mm is None or monitor_pixels is None:
        ...
        if monitor_mm is None or monitor_pixels is None:
            raise ValueError(
                'Please supply monitor dimensions manually as they could not be retrieved.')
    print(
        f'Found default monitor of size {monitor_mm[0]}x{monitor_mm[1]}mm and {monitor_pixels[0]}x{monitor_pixels[1]}px.')

    # Create PyQt visualization window
    qt_visualizer = None
    if visualize_laser_pointer:
        qt_visualizer = GazeVisualizerGUI(screen_resolution=monitor_pixels)
        
    # Setup a flag to track if application is running
    running = True
    
    # Setup exit signal handling - when the window is closed, this will be triggered
    def on_app_quit():
        nonlocal running
        running = False
        print("Application exiting due to window close...")
    ...
    plot_3d_scene = Plot3DScene(
        face_model, monitor_mm[0], monitor_mm[1], 20) if visualize_3d else None
        ...
        try:
            ...
            if results.multi_face_landmarks:
                ...
                face_landmarks = np.asarray(
                    [face_landmarks[i] for i in landmarks_ids])
                smoothing_buffer.append(face_landmarks)
                face_landmarks = np.asarray(smoothing_buffer).mean(axis=0)

                success, rvec, tvec, inliers = cv2.solvePnPRansac(
                    face_model, face_landmarks, camera_matrix, dist_coefficients, rvec=rvec, tvec=tvec, useExtrinsicGuess=rvec is not None, flags=cv2.SOLVEPNP_EPNP)  # Initial fit
                for _ in range(10):
                ...
                # data preprocessing
                face_model_transformed, face_model_all_transformed = get_face_landmarks_in_ccs(
                    camera_matrix, dist_coefficients, frame.shape, results, face_model, face_model_all, landmarks_ids)
                left_eye_center = 0.5 * \
                    (face_model_transformed[:, 2] +
                     face_model_transformed[:, 3]).reshape((3, 1))  # center eye
                right_eye_center = 0.5 * \
                    (face_model_transformed[:, 0] +
                     face_model_transformed[:, 1]).reshape((3, 1))  # center eye
                face_center = face_model_transformed.mean(axis=1).reshape((3, 1))

                img_warped_left_eye, _, _ = normalize_single_image(
                    image_rgb, rvec, None, left_eye_center, camera_matrix)
                img_warped_right_eye, _, _ = normalize_single_image(
                    image_rgb, rvec, None, right_eye_center, camera_matrix)
                img_warped_face, _, rotation_matrix = normalize_single_image(
                    image_rgb, rvec, None, face_center, camera_matrix, is_eye=False)

                if visualize_preprocessing and HAS_GUI_SUPPORT:
                    cv2.imshow('img_warped_left_eye', cv2.cvtColor(
                        img_warped_left_eye, cv2.COLOR_RGB2BGR))
                    cv2.imshow('img_warped_right_eye', cv2.cvtColor(
                        img_warped_right_eye, cv2.COLOR_RGB2BGR))
                    cv2.imshow('img_warped_face', cv2.cvtColor(
                        img_warped_face, cv2.COLOR_RGB2BGR))

                person_idx = torch.Tensor([0]).unsqueeze(0).long().to(
                    device)  # TODO adapt this depending on the loaded model
                full_face_image = transform(image=img_warped_face)[
                    "image"].unsqueeze(0).float().to(device)
                left_eye_image = transform(image=img_warped_left_eye)[
                    "image"].unsqueeze(0).float().to(device)
                right_eye_image = transform(image=img_warped_right_eye)[
                    "image"].unsqueeze(0).float().to(device)

                # prediction
                output = model(person_idx, full_face_image, right_eye_image,
                               left_eye_image).squeeze(0).detach().cpu().numpy()
                gaze_vector_3d_normalized = gaze_2d_to_3d(output)
                gaze_vector = np.dot(np.linalg.inv(
                    rotation_matrix), gaze_vector_3d_normalized)
                ...
                result = ray_plane_intersection(
                    face_center.reshape(3), gaze_vector, plane_w, plane_b)
                point_on_screen = get_point_on_screen(
                    monitor_mm, monitor_pixels, result)

                # Print gaze point to console regardless of GUI support
                if frame_idx % 30 == 0:  # Only print every 30 frames to reduce output
                    print(
                        f"Gaze point on screen: {point_on_screen[0]:.1f}, {point_on_screen[1]:.1f}")

                # Update PyQt visualization
                if qt_visualizer is not None:
                    current_fps = np.mean(fps_deque) if fps_deque else 0
                    qt_visualizer.update_gaze_point(
                        point_on_screen[0], point_on_screen[1], current_fps)

                    # Get eye metrics and update the UI
                    if hasattr(source, 'get_eye_metrics'):
                        eye_distance, blink_count, is_blinking = source.get_eye_metrics()
                        qt_visualizer.update_eye_metrics(
                            eye_distance, blink_count, is_blinking)
            ...
            # Check for ESC key (still using the safe_waitKey for compatibility)
            key = safe_waitKey(1)
            if key == 27:  # ESC key
            ...
            if not running:
                print("Window closed, exiting main loop...")
                break
        except StopIteration:
            # Handle the case when the source iterator is exhausted
            print("No more frames available, exiting loop...")
            break
    ...
    # Clean up webcam resources properly
    try:
        source.release()
    except Exception as e:
        print(f"Error while releasing webcam resources: {e}")
            
    print("Exiting application...")
    
    # Ensure application properly exits
    return app.exec_()
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument("--calibration_matrix_path",
        ...
        parser.add_argument("--visualize_laser_pointer",
        ...
        print(f"Looking for model at: {args.model_path}")
        ...
        if args.monitor_pixels is not None:
            args.monitor_pixels = tuple(
                map(int, args.monitor_pixels.split(',')))
        print('*** Monitor dimensions processed ***')

        try:
            print(f"Loading model from: {args.model_path}")

            # Fix checkpoint compatibility before loading
            fixed_checkpoint_path = fix_checkpoint_compatibility(
                args.model_path)

            # Load model from the fixed checkpoint
            model = Model.load_from_checkpoint(
                fixed_checkpoint_path).to(device)
            model.eval()
            print("Model loaded successfully!")

            # Run the main function and get the application return code
            return_code = main(args.calibration_matrix_path, args.monitor_mm, args.monitor_pixels, model,
                 args.visualize_preprocessing, args.visualize_laser_pointer, args.visualize_3d)
