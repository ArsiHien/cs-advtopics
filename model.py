...
class Model(LightningModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # pitch and yaw offset for the original and mirrored participant
        self.subject_biases = nn.Parameter(torch.zeros(15 * 2, 2))

        # Updated to use weights parameter instead of pretrained for newer torchvision
        ...
        self.cnn_face = nn.Sequential(
            ...
            models.vgg16(weights=vgg16_weights).features[:9],
            nn.Conv2d(128, 64, kernel_size=(1, 1),
                      stride=(1, 1), padding='same'),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(2, 2)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(3, 3)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 128, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(5, 5)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(128),
            nn.Conv2d(128, 128, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(11, 11)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(128),
        )
        ...
        self.cnn_eye = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 128, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(4, 5)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(128),
            nn.Conv2d(128, 128, kernel_size=(3, 3), stride=(
                1, 1), padding='valid', dilation=(5, 11)),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(128),
        )
    ...
    # Add configure_optimizers required by newer PyTorch Lightning
    ...
    def forward(self, person_idx: torch.Tensor, full_face: torch.Tensor, right_eye: torch.Tensor, left_eye: torch.Tensor):
        ...
        # t_hat + subject-dependent bias term
